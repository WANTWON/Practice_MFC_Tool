...
#pragma region 복습 1
	//CreateView : 분할 창에 표시할 VIEW 클래스들을 배치하는 함수
	//CreateView(행, 열, 배치할 VIEW 클래스 생성, 초기 크기, pContext)

	//CreateStatic :  분할 창을 만드는 함수
	//CreateStatic(부모 윈도우 핸들, 행의 수, 열의 수, 창 스타일, 자식창 id)
...
#pragma endregion 복습 2
	...
	//WS_CHILD : 자식 창이라는 것을 의미하는 윈도우 스타일을 의미
	//WS_VISIBLE : 생성 후 바로 화면에 표시하겠다는 윈도우 스타일을 의미
	...
	m_SecondSplitter.CreateView(0, 0, RUNTIME_CLASS(CMiniView), CSize(330, 170), pContext);
	m_SecondSplitter.CreateView(1, 0, RUNTIME_CLASS(CMyForm), CSize(330, 300), pContext);

	//SetColumnInfo(열 번호, 열의 크기 지정, 최소 허용 범위)
	m_MainSplitter.SetColumnInfo(0, 330, 10);
